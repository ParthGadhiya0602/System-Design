...
*One deterministic function that turns any input into a fixed-size fingerprint — and the whole trick is knowing which kind to use.*
...
- **Checksums / CRC** (CRC32, CRC-32C) — detect *accidental* corruption (bit flips from a flaky disk or network), not deliberate tampering. Fast, often hardware-accelerated; found in Ethernet frames, ZIP entries, gzip streams.

<br>

**Consistent hashing — hashing applied to distribute data.**
Map both keys *and* nodes onto a circular hash space (a "ring"); a key belongs to the first node clockwise of it. Add or remove a node and only its immediate neighbor's keys move — not the whole cluster.
...
1. Why is a fast cryptographic hash like SHA-256 still the *wrong* tool for storing passwords, and what should you use instead?
