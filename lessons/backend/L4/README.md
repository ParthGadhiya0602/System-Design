...
**Just getting started** -- 11 of 15 topics written. Work through them in order; each is a short, self-contained read (~5-8 min).
...
| 09 | [Event sourcing](09-event-sourcing.md) | Storing every change as an immutable event instead of overwriting state, so the log itself is the source of truth. | ✅ |
| 10 | [CQRS](10-cqrs.md) | Splitting the read model from the write model so each can be optimized for what it actually does. | ✅ |
